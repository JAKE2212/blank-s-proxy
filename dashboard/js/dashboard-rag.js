...
async function ragToggle() {
  try {
    ...
    if (data.ok) { _ragConfig = data.config; fetchRagStatus(); }
  } catch (e) {
...
async function ragBlindNext() {
  try {
    ...
    if (data.ok) {
      ...
      const orig = { text: btn.textContent, bg: btn.style.background, border: btn.style.borderColor, color: btn.style.color };
...
async function fetchRagCollections() {
  ...
  try {
    ...
    if (!data.ok || !data.collections.length) {
      el.innerHTML = '<div class="empty-state">No collections yet. Start a roleplay to create one.</div>';
      return;
    }
...
async function ragClearCollection(name) {
  if (!await showConfirm(`Clear all memory for "${name}"? This cannot be undone.`)) return;
  try {
    const data = await fetch(`/extensions/rag/collections/${encodeURIComponent(name)}`, { method: 'DELETE' }).then(r => r.json());
...
function ragOnSlider(key) {
  ...
  if (def && val !== undefined) document.getElementById(`rag-vl-${key}`).textContent = def.fmt(val);
}
