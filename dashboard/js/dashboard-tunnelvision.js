...
function renderEntries(entries) {
  if (!entries.length) return '<div style="font-size:0.72rem;color:#8aaaaa;padding:4px 0;">No entries yet.</div>';
...
async function tvDeleteTree(name) {
  if (!await showConfirm(`Delete tree "${name}"? This cannot be undone.`)) return;
  try {
...
async function saveTvEntry() {
  ...
  try {
    const data = await fetch(
      ...
      { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content, keys }) }
    ).then(r => r.json());
    if (!data.ok) { errEl.textContent = data.error || 'Failed.'; errEl.style.display = 'block'; return; }
