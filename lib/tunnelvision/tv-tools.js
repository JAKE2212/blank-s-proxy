...
function buildTrackerList(tree) {
  const trackers = [];
  const { getAllEntries } = require("./tv-tree");
...
function notebookAction(tree, args, botName) {
  ...
  switch (action) {
    ...
    case "promote": {
      ...
      try {
        const { addEntry } = require("./tv-tree");
...
function mergeSplitAction(tree, args) {
  ...
  if (action === "merge") {
    ...
    try {
      const { findEntry, updateEntry, disableEntry } = require("./tv-tree");
  ...
  if (action === "split") {
    ...
    try {
      const { findEntry, updateEntry, addEntry } = require("./tv-tree");
...
function reorganizeAction(tree, args) {
  ...
  switch (action) {
    case "move_entry": {
      ...
      try {
        const { moveEntry } = require("./tv-tree");
    ...
    case "move_node": {
      ...
      try {
        const { moveNode } = require("./tv-tree");
