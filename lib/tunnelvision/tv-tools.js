...
function buildTrackerList(tree) {
  ...
  for (const { entry } of getAllEntries(tree)) {
    if (entry.isTracker || entry.title.match(/^\[Tracker\]/i)) {
...
// ── Tool: Search (Traversal mode) ────────────────────────────────────────────

const TRAVERSAL_THRESHOLD = 15; // nodes — above this, use traversal instead of collapsed

/**
 * Build the traversal Search tool definition.
 * Shows only top-level channels. AI must drill down step by step.
 * @param {object} tree
 * @param {string} [nodeId] — if provided, shows children of this node instead of root
 * @returns {object}
 */
function searchTraversalDefinition(tree, nodeId) {
  const trackers = buildTrackerList(tree);
  const targetId = nodeId ?? tree.rootId;
  const targetNode = tree.nodes[targetId];

  if (!targetNode) return searchDefinition(tree); // fallback to collapsed

  // Build channel listing for this level only
  const channels = [];
  const directEntries = targetNode.entries.filter(e => e.enabled !== false);

  if (directEntries.length > 0) {
    channels.push(`  📄 ${directEntries.length} direct entries at this level`);
  }

  for (const cid of targetNode.children) {
    const child = tree.nodes[cid];
    if (!child) continue;
    let entryCount = 0;
    const { walkTree } = require("./tv-tree");
    walkTree(tree, (n) => { entryCount += n.entries.filter(e => e.enabled !== false).length; }, cid);
    const hasChildren = child.children.length > 0;
    const suffix = hasChildren ? " [has sub-channels]" : "";
    const summary = child.summary ? ` — ${child.summary}` : "";
    channels.push(`  📡 [${cid}] ${child.label} (${entryCount} entries)${suffix}${summary}`);
  }

  const locationStr = targetId === tree.rootId
    ? `📺 TunnelVision — ${tree.charName} (Top Level)`
    : `📺 TunnelVision — ${tree.charName} > ${targetNode.label}`;
...
USAGE:
- Pick node IDs to either RETRIEVE their entries or DRILL DEEPER into sub-channels
- Channels marked [has sub-channels] can be navigated into for more specific content
- You can retrieve multiple nodes in a single call
- Always search before Remember to avoid creating duplicate entries`,
      parameters: {
        type: "object",
        properties: {
          node_ids: {
            type: "array",
            items: { type: "string" },
            description: "Array of node IDs to retrieve or navigate into.",
          },
          reason: {
            type: "string",
            description: "Brief explanation of why these nodes are relevant.",
          },
        },
        required: ["node_ids"],
      },
      ...
      parameters: {
        ...
        properties: {
          ...
          isTracker: {
            type: "boolean",
            description:
              "Set to true if this entry should be actively checked and updated every turn (e.g. mood tracker, inventory, relationship status). Tracker entries appear in tool descriptions as reminders.",
          },
        },
...
If this is genuinely new/different information, call TunnelVision_Remember again with force=true to proceed anyway.`;
  ...
  try {
    const entry = addEntry(tree, targetId, {
      ...
      keys,
      isTracker: args.isTracker ?? false,
    });
 ...
 */
function buildToolDefinitions(tree, opts = {}) {
  const nodeCount = Object.keys(tree.nodes).length;
  const useTraversal = opts.forceTraversal ?? (nodeCount >= TRAVERSAL_THRESHOLD);
  const searchDef = useTraversal
    ? searchTraversalDefinition(tree)
