...
function addEntry(tree, nodeId, params) {
  ...
  const entry = {
    ...
    enabled: true,
    isTracker: params.isTracker ?? false,
    createdAt: t,
    updatedAt: t,
  };
...
/**
 * Run diagnostic checks on a tree. Returns an array of issues found,
 * with auto-fix applied where possible.
 * @param {object} tree
 * @returns {{ issues: Array<{ level: string, message: string, fixed: boolean }>, nodeCount: number, entryCount: number, trackerCount: number }}
 */
function runDiagnostics(tree) {
  const issues = [];
  let fixApplied = false;

  // Check 1: Root node exists
  if (!tree.nodes[tree.rootId]) {
    issues.push({ level: "critical", message: "Root node missing", fixed: false });
  }

  // Check 2: Summaries node exists
  if (!tree.nodes[tree.summariesId]) {
    issues.push({ level: "warning", message: "Summaries node missing — recreating", fixed: true });
    const summaries = makeNode("summaries", "Summaries", "root");
    tree.nodes.summaries = summaries;
    if (tree.nodes[tree.rootId] && !tree.nodes[tree.rootId].children.includes("summaries")) {
      tree.nodes[tree.rootId].children.push("summaries");
    }
    fixApplied = true;
  }

  // Check 3: Orphaned nodes (parentId points to non-existent node)
  for (const [id, node] of Object.entries(tree.nodes)) {
    if (id === tree.rootId) continue;
    if (node.parentId && !tree.nodes[node.parentId]) {
      issues.push({ level: "warning", message: `Node "${node.label}" [${id}] has orphaned parent "${node.parentId}" — moving to root`, fixed: true });
      node.parentId = tree.rootId;
      if (!tree.nodes[tree.rootId].children.includes(id)) {
        tree.nodes[tree.rootId].children.push(id);
      }
      fixApplied = true;
    }
  }

  // Check 4: Children arrays reference non-existent nodes
  for (const [id, node] of Object.entries(tree.nodes)) {
    const validChildren = node.children.filter(cid => tree.nodes[cid]);
    if (validChildren.length !== node.children.length) {
      const removed = node.children.length - validChildren.length;
      issues.push({ level: "warning", message: `Node "${node.label}" [${id}] had ${removed} stale child reference(s) — cleaned`, fixed: true });
  ...
  for (const node of Object.values(tree.nodes)) {
    for (const entry of node.entries) {
      if (uidMap.has(entry.uid)) {
        issues.push({ level: "error", message: `Duplicate UID ${entry.uid}: "${entry.title}" and "${uidMap.get(entry.uid)}"`, fixed: false });
      } else {
        uidMap.set(entry.uid, entry.title);
      }
    }
  }

  // Check 6: nextUid is higher than all existing UIDs
  const maxUid = Math.max(0, ...uidMap.keys());
  if (tree.nextUid <= maxUid) {
    issues.push({ level: "warning", message: `nextUid (${tree.nextUid}) is <= max existing UID (${maxUid}) — fixing to ${maxUid + 1}`, fixed: true });
    tree.nextUid = maxUid + 1;
    fixApplied = true;
  }

  // Check 7: Entries with empty content
  for (const node of Object.values(tree.nodes)) {
    for (const entry of node.entries) {
      if (entry.enabled && (!entry.content || !entry.content.trim())) {
        issues.push({ level: "info", message: `Entry UID ${entry.uid} "${entry.title}" has empty content`, fixed: false });
      }
    }
  }

  // Check 8: Very large entries (>5000 chars)
  for (const node of Object.values(tree.nodes)) {
    for (const entry of node.entries) {
      if (entry.enabled && entry.content && entry.content.length > 5000) {
        issues.push({ level: "info", message: `Entry UID ${entry.uid} "${entry.title}" is very large (${entry.content.length} chars) — consider splitting`, fixed: false });
      }
    }
  }
  ...
  // Collect stats
  const allEntries = getAllEntries(tree);
  const trackerCount = allEntries.filter(e => e.entry.isTracker || e.entry.title.match(/^\[Tracker\]/i)).length;
  ...
  return {
    ...
    trackerCount,
    healthy: issues.every(i => i.level === "info" || i.fixed),
  };
}
